...
const prisma = () => ({
  user: { create: spy(async () => ({ id: 'user-1', name: 'Guest User', email: null, isGuest: true })) },
  task: { createMany: spy(async () => ({ count: 3 })) },
  session: {
    create: spy(async () => ({})),
...
test('AuthService creates a guest session and seeds starter tasks', async () => {
  ...
  assert.equal(db.task.createMany.calls[0][0].data.length, 6);
});
